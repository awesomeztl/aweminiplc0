	...
	while (true)
	{
		...
		case UNSIGNED_INTEGER_STATE:
		{
			...
			if (!current_char.has_value())
			{
				...
				if (ss >> _int)
				{
					return std::make_pair(std::make_optional<Token>(TokenType::UNSIGNED_INTEGER, _int, pos, currentPos()), std::optional<CompilationError>());
				}
				else
				{
		...
		case IDENTIFIER_STATE:
		{
			...
			{
				ss >> _STRING;
				////根据currentPos获得当前该字符的位置，作为返回的编译错误的位置（自己标注）
				if (_STRING == "begin")
				{
					return std::make_pair(
						std::make_optional<Token>(TokenType::BEGIN, _STRING, pos, currentPos()),
						std::optional<CompilationError>());
				}
				else if (_STRING == "end")
				{
					return std::make_pair(std::make_optional<Token>(TokenType::END, _STRING, pos, currentPos()),
										  std::optional<CompilationError>());
				}
				else if (_STRING == "const")
				{
					return std::make_pair(
						std::make_optional<Token>(TokenType::CONST, _STRING, pos, currentPos()),
						std::optional<CompilationError>());
				}
				else if (_STRING == "var")
				{
					return std::make_pair(std::make_optional<Token>(TokenType::VAR, _STRING, pos, currentPos()),
										  std::optional<CompilationError>());
				}
				else if (_STRING == "print")
				{
					return std::make_pair(
						std::make_optional<Token>(TokenType::PRINT, _STRING, pos, currentPos()),
						std::optional<CompilationError>());
				}
				else
				{
					return std::make_pair(
						std::make_optional<Token>(TokenType::IDENTIFIER, _STRING, pos, currentPos()),
						std::optional<CompilationError>());
				}
			}
...
////检查标识符函数，如果这个标识符以数字开头，就返回是无效的标识符
