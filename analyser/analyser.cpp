...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
		...
		while (true) {
			...
			{
				addUninitializedVariable(token.value());
				continue;
			}
			else if (next.value().GetType() == TokenType::EQUAL_SIGN)
			{
				...
				int32_t index = getIndex(token.value().GetValueString());
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
		...
		if (!next.has_value() || next.value().GetType() != TokenType::IDENTIFIER)
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNeedIdentifier);
		if (!isDeclared(next.value().GetValueString())) //是否声明过
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNotDeclared);
		if (isConstant(next.value().GetValueString())) //是否常量
		...
		switch (next.value().GetType()) {
			...
			case IDENTIFIER:{
				if(isUninitializedVariable(next.value().GetValueString()))
				 ...
				 _Int = getIndex(next.value().GetValueString());
				_instructions.emplace_back(Operation::LOD, _Int);
				break;
			}
			case UNSIGNED_INTEGER:
			{
				...
				int32_t _int;
				ss >> _int;
				_instructions.emplace_back(Operation::LIT, _int);
				break;
			}
			case LEFT_BRACKET:
